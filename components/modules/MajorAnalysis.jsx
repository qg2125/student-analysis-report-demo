...
const MajorAnalysis = () => {
  ...
  const majorData = {
    计算机科学: {
      overview:
        "CS作为应用性极强的工程类学科，向来是工程学院中最热门的专业之一。现代CS学科又在以前所未有的速度和广度，与其他众多学科，包括数学、计算科学、医学、材料学等等不断地交错互动，逐渐成为在全世界范围内影响力最大最广的学科之一。",
      details:
        "目前距离广义申业还有一段距离，如果完全确定一个领域，可能无法预测将来的发展趋势，但可以尽可能跟紧潮流。建议DS和CS混合申请，这两个专业不需要比赛成绩很优异才可以在这个领域有所建树。比赛本身类似考试场景，对于算法、数据结构作出思维拓展。DS需要从CS的核心课去学习，首先要学会精通一个编程语言，也就是Java, C++, python至少精通一个。适应面较广，学会一个之后其他语言会快很多上手。算法，离散数学，数据结构也建议精通，打好基础。建议数据库SQL，大数据也学精，计算机系统（导论）也建议学习掌握。基础打好之后，机器学习是重中之重。拓展选修课：计算机网络，操作系统，分布式系统，网络安全等。",
      programs:
    ...
    数据科学: {
      overview:
        "DS一般设置1-2年，建议选择时间较长的项目，方便进行跟踪+积淀技术经验。工作、大厂点的DS项目下面（100人+），会细分很多方向，如用它研究任何问题，DS in Physics，DS in Biology，生物信息学，Social Science等；或按照数学文字图片视频等分类，如：NLP（需要学习语言学课程），CV（图片处理），小的项目则区分为偏应用/偏理论的分支。",
      details:
        "DS属于交叉学科，本科很少开设，大部分申请者的本科背景是统计、数学、计算机、金融等。该专业看重学生对数学统计基础，要求CS能力，但要求都不那么高，需要熟练使用编程语言和软件，尤其是：微积分、线代、概率论和统计，各科得分最好是B以上；时间序列属于略丰富性的课程，编程要求：需要掌握数据库，数据结构，算法，对于学习DL，ML来说，需要会使用R<python，用python进行数据读取，分析，清理，做数据可视化，看重分析的能力，不太需要C++等。Model：用NumPy，pandas等，也可以学习Pytorch（一个开源的Python机器学习库，基于Torch，用于自然语言处理等应用程序）。需要熟悉这些常用的工具，会建模、解决问题，建议申请时，有2-3个DS相关经历，如：实习，校内科研，课内项目都可以。",
      programs:
